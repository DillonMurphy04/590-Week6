...
-   Before training, weights will be initialized. There are multiple ways for this.
    ...
    -   If weights all initialized to 0 or the same constant value leasd to symmetry problem, since if all the weights were initialized to the same value, the neurons in each layer would learn the same features and update symmetrically during training, which would lead to inefficient training.
    ...
    Hover over the dashed lines between the neurons to see the initialized weights.
...
-   After calculating the weighted sum, an activation function (e.g., ReLU, Sigmoid, or Tanh) is applied to introduce non-linearity into the model
